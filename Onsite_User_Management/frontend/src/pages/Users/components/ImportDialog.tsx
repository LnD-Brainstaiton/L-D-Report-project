...
const ImportDialog: React.FC<ImportDialogProps> = ({
  ...
  const getErrorText = (error: { error?: string } | string): string => {
      ...
      <DialogContent>
        <Box display="flex" flexDirection="column" gap={2} mt={1}>
          <Typography variant="body2" color="text.secondary">
            Upload an Excel (.xlsx) or CSV file to import employees. The file should contain columns: Employee ID, Name,
            Email, Department, Designation, Career Start Date, BS Joining Date.
          </Typography>
          ...
          <Box>
            ...
            >
              ...
              <IconButton size="small">{showPreview ? <ExpandLess /> : <ExpandMore />}</IconButton>
            </Box>

            <Collapse in={showPreview}>
              <Box
                mt={1}
                sx={{ border: `1px solid ${alpha(theme.palette.divider, 0.1)}`, borderRadius: 2, overflow: 'hidden' }}
              >
                <Box
                  sx={{
                    p: 1,
                    ...
                    <TableBody>
                      {previewData.map((row, index) => (
                        ...
                        >
                          ...
                          <TableCell sx={{ fontSize: '0.75rem', whiteSpace: 'nowrap' }}>
                            {row.career_start_date || 'N/A'}
                          </TableCell>
                          <TableCell sx={{ fontSize: '0.75rem', whiteSpace: 'nowrap' }}>
                            {row.bs_join_date || 'N/A'}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
          ...
          <label htmlFor="import-file-input">
            <Button variant="outlined" component="span" fullWidth startIcon={<UploadFile />} sx={{ mb: 2 }}>
