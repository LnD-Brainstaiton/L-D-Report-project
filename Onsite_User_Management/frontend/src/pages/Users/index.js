...
import { handleCreateStudent, handleRemoveEmployee, handleImportExcel, handleImportCSV, handleGenerateOverallReport } from './utils/userHandlers';
...
function Users() {
  ...
  const confirmRemove = async () => {
    await handleRemoveEmployee(selectedUserToRemove, setMessage, setRemoveDialogOpen, setSelectedUserToRemove, fetchUsers);
  };
                            ...
                            {user.enrollments && user.enrollments.length > 0 ? (
                              <Box display="flex" flexDirection="column" gap={2}>
                                {user.enrollments
                                  ...
                                  .map((enrollment) => (
                                    <CourseHistoryCard key={enrollment.id} enrollment={enrollment} />
                                  ))}
                              </Box>
                            ) : (
