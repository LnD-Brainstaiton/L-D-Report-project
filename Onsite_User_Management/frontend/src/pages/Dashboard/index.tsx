...
import type { CourseType, TimePeriod } from '../../types';
...
const Dashboard: React.FC = () => {
  ...
  const [displayCounts, setDisplayCounts] = useState<DisplayCounts>({
    upcoming: 10,
    ongoing: 10,
    planning: 10,
    completed: 10,
  ...
  const dateRangeFormatted = formatDateRange(timePeriod, selectedMonth, selectedQuarter, selectedYear);
    ...
    <Box sx={{ p: 3 }}>
      <Box mb={4} display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
        ...
        <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
            ...
            <>
              ...
              >
                {[
                  ...
                  <MenuItem key={index} value={String(index)}>{month}</MenuItem>
                ))}
              </TextField>
              <TextField
                type="number"
          ...
          {timePeriod !== 'all' && (
            <Chip icon={<CalendarToday />} label={dateRangeFormatted} color="primary" variant="outlined" />
          )}
        </Box>
      </Box>
