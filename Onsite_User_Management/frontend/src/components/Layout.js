...
function Layout({ children }) {
  ...
  useEffect(() => {
    if (location.pathname.startsWith('/courses/')) {
      setCoursesOpen(true);
      // Determine which course type submenu to open
      const pathParts = location.pathname.split('/');
      if (pathParts.length >= 3) {
        const courseType = pathParts[2]; // onsite, online, or external
        if (courseType === 'onsite' && pathParts.length > 3) {
          // Only open onsite submenu if there's a status (planning, upcoming, etc.)
          setOnsiteOpen(true);
        }
      }
    }
